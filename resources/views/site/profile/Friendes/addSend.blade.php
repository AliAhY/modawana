        ...
        <div class="container mt-5">
            <div class="tab-content" id="pills-tabContent">
                <div class="tab-pane fade show active" id="pills-friends" role="tabpanel"
                    ...
                    <h3 class="mb-3 fw-semibold">Add Friends To : <span
                            class="badge text-bg-secondary fs-2 rounded-4 py-1 px-2 ms-2">{{ $num_of_frinds_add }}</span>
                    </h3>
                    <div class="row">
                        @foreach ($all_friends_adds as $request)
                            <div class="col-sm-6 col-md-4 mb-4">
                                <div class="card p-2">
                                    <div class="position-relative">
                                        <div class="rounded-circle overflow-hidden"
                                            style="width: 150px; height: 150px; margin: 0 auto;">
                                            @if ($request->recipient->avatar == null)
                                                @if ($request->recipient->gender == 'male')
                                                    <a
                                                        href="{{ route('profile.other', [$request->recipient->name, $request->recipient->id]) }}">
                                            ...
                                            @else
                                                @php
                                                    // تحليل قيمة avatar (JSON) لاسترداد اسم الملف
                                                    $avatarData = json_decode($request->recipient->avatar);
                                                    $filename = $avatarData->filename ?? null; // التأكد من وجود البيانات
                                                @endphp
                                                ...
                                                <a
                                                    href="{{ route('profile.other', [$request->recipient->name, $request->recipient->id]) }}">
                                                    <img src="{{ url('/storage/media/users/User_ID_' . $request->recipient->user_id . '/images/profile/' . $filename) }}"
                                                        alt="Avatar Photo" class="w-100 h-100"></a>
                                            @endif
                                        </div>
                                        {{-- <h5 class="text-center mt-2">{{ $request->recipient->name }}</h5> --}}
                                        @if ($request->recipient)
                                            <h5 class="text-center">
                                                <a href="{{ route('profile.other', [$request->recipient->name, $request->recipient->id]) }}"
                                                    class="profile-link">
